...
def predict():
    ...
    #float_features = [float(x) for x in dict(request.form).values()]
    final_features = [dict(request.form)['a'] , dict(request.form)['b'], dict(request.form)['c']]
    ...
    return jsonify({"response" : final_features[1]})
